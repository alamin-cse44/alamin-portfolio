...
export function Experience() {
  return (
    <section id="experience" className="relative z-10 py-24 md:py-32 border-t border-border-soft">
          ...
          {experience.map((job) => (
            <div key={job.company} className="relative pb-14 last:pb-0">
              ...
              {"link" in job && job.link && (
                <a href={job.link} target="_blank" rel="noopener" className={`${buttonVariants({ variant: "outline", size: "sm" })} inline-block mt-5`}>
                  View project ↗
                </a>
              )}
            </div>
